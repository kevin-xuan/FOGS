...
def historical_average_predict(df, period=12 * 24 * 7, test_ratio=0.2, null_val=0.):
    ...
    :return:
    """
    n_sample, n_sensor = df.shape  # n_sensor是node(n_road)
    ...
    return y_predict, y_test  # (n_test, n_road)
...
def eval_static(traffic_reading_df):
    ...
    for horizon in horizons:
        ...
        # y_predict = y_predict.iloc[:, :].values
        ...
        print(line)
        # logger.info(line)
        # print('RMSE:{:.4f}, MAPE:{:.4f}, MAE:{:.4f}'.format(rmse, mape, mae))
